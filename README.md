# Laboratorio de Lenguajes de Programación I

![Made with Love](https://img.shields.io/badge/Made%20with-Love-pink?style=for-the-badge&logo=data:image/svg%2bxml;base64,)

Tarea 1 del laboratorio de Lenguajes de Programacion I (CI-3661) de la Universidad Simón Bolívar.
Este tarea 1 se implementará en **Haskell** para practicar el paradigma de la programación funcional.

## Ejecución

Para compilar y ejecutar el proyecto de la tarea 1, usaremos `stack`, por lo que para compilar y ejecutar las pruebas, simplemente debemos correr los siguientes comandos en la terminal, ubicados en la raíz del proyecto:

```bash
stack build
stack test
```

Realmente no se agrego ninguna funcionalidad al archivo `src/Main.hs`, es por esto que lo principal que se debe ejecutar son las pruebas con el comando `stack test` para verificar que las funciones implementadas en `src/Tarea1.hs` funcionan correctamente.

## Estudiante

- Luis Isea [@lmisea](https://github.com/lmisea) (19-10175).
