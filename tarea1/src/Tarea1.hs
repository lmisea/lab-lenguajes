...
-- Nombre del estudiante: [Luis Isea]
-- Carnet: [19-10175]
-------------------------------------------------------------------------------
...
parsearCondicional ss = map parseOne ss
    where
        -- Función auxiliar para parear un Int de manera segura
        tryParseInt :: String -> Maybe Int
        tryParseInt s = readMaybe s

        -- Función auxiliar para convertir una cadena a mayúsculas
        toUpperStr :: String -> String
        toUpperStr = map toUpper

        parseOne s = case tryParseInt s of
            Just n  -> Right n
            Nothing -> Left (toUpperStr s)
